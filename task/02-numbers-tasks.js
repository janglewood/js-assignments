...
function roundToPowerOfTen(num, pow) {
    ...
    let base = Math.floor(num / 10 ** pow) * 10 ** pow;
    let remainder = num % 10 ** pow;

    return remainder < 5 * (10 ** (pow -  1)) ? base + 0 : base + 10 ** pow;
}
...
function toNumber(value, def) {
    return typeof Number(value) === 'number' && Number(value) + '' !== 'NaN' ? Number(value) : def;
}
